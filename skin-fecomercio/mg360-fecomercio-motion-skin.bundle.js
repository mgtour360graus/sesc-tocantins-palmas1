...
.mg360-float-action{position:absolute;z-index:10;color:#fff;background:transparent!important;display:flex;align-items:center;flex-direction:column;gap:6px;filter:drop-shadow(0 8px 13px rgba(0,33,66,.28));transition:transform .25s ease}.mg360-float-action:hover{transform:scale(1.06)}.mg360-float-action .mg360-circle{display:grid;place-items:center;width:58px;height:58px;border-radius:50%;background:var(--mg-sky);border:2px solid #fff;box-shadow:0 0 0 5px rgba(0,76,153,.32);font-size:27px;font-weight:300}.mg360-float-action em{font-size:8px;font-style:normal;font-weight:900;letter-spacing:.12em;padding:5px 9px;border-radius:4px;background:var(--mg-blue)}.mg360-float-info{left:63%;top:46%}.mg360-float-next{right:20%;top:27%}.mg360-float-next .mg360-circle svg{width:25px;height:25px}\
...
@media(prefers-reduced-motion:reduce){#mg360-ui *,#mg360-ui *:before,#mg360-ui *:after{animation-duration:.01ms!important;transition-duration:.01ms!important;animation-iteration-count:1!important}}';
    ...
    root.innerHTML = '\
      ...
      <button class="mg360-float-action mg360-float-info" type="button" aria-label="Saiba mais sobre o ambiente"><span class="mg360-circle">+</span><em>Saiba mais</em></button>\
      <button class="mg360-float-action mg360-float-next" type="button" aria-label="Ir à próxima panorâmica"><span class="mg360-circle">' + icon('arrow') + '</span><em>Próximo</em></button>\
    ...
    root.querySelector('.mg360-float-info').addEventListener('click', openInfo);
    root.querySelector('.mg360-float-next').addEventListener('click', function () { step(1); });
